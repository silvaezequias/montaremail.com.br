...
export default function Sidebar({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0f0f0f] text-zinc-100 border-r border-zinc-800">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {/* Presets / Templates */}
        <div className="border border-zinc-800 bg-zinc-900/10 rounded-xl overflow-hidden transition-all">
          <button
            type="button"
            onClick={() => toggleSection('quickTemplates')}
          ...
          >
            ...
            {openSections.quickTemplates ? <ChevronUp className="h-4 w-4 text-zinc-500" /> : <ChevronDown className="h-4 w-4 text-zinc-500" />}
          </button>

          {openSections.quickTemplates && (
            <div className="p-3 space-y-2 bg-[#0c0c0c]/40 border-t border-zinc-900">
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => onLoadPreset('welcome')}
                  className="py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span>Boas-vindas</span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-1 leading-tight">E-mail onboarding</span>
                </button>
                <button
                  onClick={() => onLoadPreset('promo')}
                  className="py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span>Promoção</span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-1 leading-tight">Cupom & Desconto</span>
                </button>
                <button
                  onClick={() => onLoadPreset('scheduling')}
                  className="py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span>Convite/Agenda</span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-1 leading-tight">Com botão de RSVP</span>
                </button>
                <button
                  onClick={() => onLoadPreset('otp_code')}
                  className="py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span>Confirmação OTP</span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-1 leading-tight">Código de segurança</span>
                </button>
                <button
                  onClick={() => onLoadPreset('account_created')}
                  className="col-span-2 py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between"
                >
                  <span>Conta Criada</span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-0.5 leading-tight">Aviso de cadastro e onboarding</span>
                </button>

                <div className="text-[10px] uppercase font-bold text-zinc-500 tracking-wider pt-2.5 pb-1 border-t border-zinc-800/60 col-span-2 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
                  Modelos Premium (Inspirados na Imagem)
                </div>

                <button
                  onClick={() => onLoadPreset('barebone')}
                  className="py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span className="flex items-center gap-1">⚄ Barebone</span>
                  <span className="text-[9px] text-zinc-400 font-normal mt-1 leading-tight">Minimalista moderno, contraste limpo</span>
                </button>

                <button
                  onClick={() => onLoadPreset('matte')}
                  className="py-2 px-2.5 bg-emerald-950/20 hover:bg-emerald-950/30 border border-emerald-900/40 rounded-lg text-left text-xs font-medium text-emerald-300 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span className="flex items-center gap-1">🌿 Matte</span>
                  <span className="text-[9px] text-emerald-500/80 font-normal mt-1 leading-tight">Estilo Collage verde orgânico</span>
                </button>

                <button
                  onClick={() => onLoadPreset('protocol')}
                  className="py-2 px-2.5 bg-zinc-950 hover:bg-zinc-900 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-100 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span className="flex items-center gap-1">⧁ Protocol</span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-1 leading-tight">Cyber tech escuro de alta segurança</span>
                </button>

                <button
                  onClick={() => onLoadPreset('arcane')}
                  className="py-2 px-2.5 bg-rose-950/20 hover:bg-rose-950/30 border border-rose-900/40 rounded-lg text-left text-xs font-medium text-rose-300 transition-colors cursor-pointer flex flex-col justify-between min-h-[64px]"
                >
                  <span className="flex items-center gap-1">🌹 Arcane</span>
                  <span className="text-[9px] text-rose-500/80 font-normal mt-1 leading-tight">Editorial romântico burgundy</span>
                </button>

                <button
                  onClick={() => onLoadPreset('studio')}
                  className="col-span-2 py-2.5 px-3 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-left text-xs font-medium text-zinc-200 transition-colors cursor-pointer flex flex-col justify-between"
                >
                  <span className="flex items-center gap-1">💍 Studio (Halo Chrome)</span>
                  <span className="text-[9px] text-zinc-400 font-normal mt-1 leading-tight">Tech industrial com design em tons de creme e cromo líquido</span>
                </button>

                <div className="border-t border-zinc-800/60 col-span-2 pt-1.5" />

                <button
                  onClick={() => onLoadPreset('empty')}
                  className="col-span-2 py-2 px-2.5 bg-zinc-900 hover:bg-zinc-850 border border-dashed border-red-900/40 text-red-400 hover:text-red-300 rounded-lg text-left text-xs font-bold transition-colors cursor-pointer flex flex-col justify-between mt-1"
                >
                  <span className="flex items-center gap-1">
                    <RotateCcw className="h-3.5 w-3.5 text-red-400 animate-spin" /> Começar do Zero (Em Branco)
                  </span>
                  <span className="text-[9px] text-zinc-500 font-normal mt-0.5 leading-tight">Limpar o canvas inteiro e iniciar com design em branco</span>
                </button>
              </div>
            </div>
          )}
        ...
        <div className="border border-zinc-800 bg-zinc-900/10 rounded-xl overflow-hidden transition-all">
          <button
            type="button"
            onClick={() => toggleSection('savedTemplates')}
            className="w-full px-4 py-3 bg-zinc-900/60 hover:bg-zinc-850/80 flex justify-between items-center text-xs font-bold text-zinc-300 uppercase tracking-wider transition-colors cursor-pointer border-b border-zinc-800/40"
          >
            <span className="flex items-center gap-1.5">
            ...
            {openSections.savedTemplates ? <ChevronUp className="h-4 w-4 text-zinc-500" /> : <ChevronDown className="h-4 w-4 text-zinc-500" />}
          </button>

          {openSections.savedTemplates && (
            <div className="p-3 space-y-3 bg-[#0c0c0c]/40 border-t border-zinc-900">
              <div className="flex justify-between items-center pb-1 border-b border-zinc-850/30">
                <span className="text-[10px] text-zinc-500">Armazenamento Local</span>
                <button
                  onClick={onOpenStorageManager}
                  className="text-[9px] bg-zinc-850 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-zinc-200 px-2 py-0.5 rounded-md transition-all cursor-pointer flex items-center gap-1 font-semibold"
                >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-1.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-[10px] font-semibold text-zinc-300 flex items-center justify-center gap-1 transition-colors hover:text-white"
                >
                  <Save className="h-3 w-3 text-blue-400" />
                  Salvar Atual
                </button>
                <label className="flex-1 py-1.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 rounded-lg text-[10px] font-semibold text-zinc-300 flex items-center justify-center gap-1 cursor-pointer transition-colors hover:text-white text-center">
              ...
              {savedTemplates.length === 0 ? (
                <div className="text-center py-4 border border-dashed border-zinc-800 rounded-lg text-zinc-500 text-[10px] bg-zinc-950/20">
                ...
                <div className="space-y-1 max-h-[140px] overflow-y-auto pr-1">
                  {savedTemplates.map((t) => {
                    ...
                    return (
                      <button
                        key={t.id}
                        onClick={() => onLoadSavedTemplate(t.id)}
                        className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg border text-left text-xs transition-colors cursor-pointer ${
                          isActive
                      ...
                      >
                        ...
                        <span className="text-[8px] text-zinc-500 font-mono">
                          {(new Blob([JSON.stringify(t)]).size / 1024).toFixed(1)} KB
                        </span>
                      </button>
                    );
                  })}
        ...
        <div className="border border-zinc-800 bg-zinc-900/10 rounded-xl overflow-hidden transition-all animate-fade-in">
          <button
            type="button"
          ...
          {openSections.iconsGallery && (
            <div className="p-3.5 space-y-4 bg-[#0c0c0c]/40 border-t border-zinc-900">
              ...
              <div className="bg-zinc-900/40 p-2.5 rounded-lg border border-zinc-800/60 space-y-3 text-xs">
                ...
                <div className="space-y-1">
                  <label className="text-[9px] text-zinc-500 font-bold uppercase block">Cor do Ícone</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      value={iconColor}
                      onChange={(e) => setIconColor(e.target.value)}
                      className="w-6 h-6 rounded cursor-pointer border border-zinc-800 bg-transparent shrink-0"
                    />
                    <input
                      type="text"
                      value={iconColor}
                      onChange={(e) => setIconColor(e.target.value)}
                      className="w-full text-[10px] font-mono bg-zinc-950 border border-zinc-850 rounded px-2 py-1 text-zinc-300 focus:outline-none"
                    />
                  </div>
                  {/* Brand colors shortcuts */}
                  {visualIdentity.brandColors.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {visualIdentity.brandColors.map((bc) => (
                        <button
                          key={bc.id}
                          onClick={() => setIconColor(bc.value)}
        ...
        <div className="border border-zinc-800 bg-zinc-900/10 rounded-xl overflow-hidden transition-all">
          ...
          {openSections.blocks && (
            <div className="p-3 space-y-2 bg-[#0c0c0c]/40 border-t border-zinc-900 max-h-[300px] overflow-y-auto pr-1">
        ...
        <div className="border border-zinc-800 bg-zinc-900/10 rounded-xl overflow-hidden transition-all">
          ...
          {openSections.structure && (
            <div className="p-3 bg-[#0c0c0c]/40 border-t border-zinc-900">
              {elements.length === 0 ? (
        ...
        <div className="border border-zinc-800 bg-zinc-900/10 rounded-xl overflow-hidden transition-all">
          ...
          {openSections.globalStyles && (
            <div className="p-3.5 space-y-4 bg-[#0c0c0c]/40 border-t border-zinc-900">
              {/* Background Canvas */}
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-zinc-400">Fundo da Tela</label>
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={globalStyles.backgroundColor}
                    onChange={(e) => onUpdateGlobalStyles({ ...globalStyles, backgroundColor: e.target.value })}
                    className="w-6.5 h-6.5 rounded border border-zinc-800 bg-transparent cursor-pointer"
                  />
                  <span className="text-xs font-mono text-zinc-400 uppercase">{globalStyles.backgroundColor}</span>
                </div>
              </div>

              {/* Background Card */}
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-zinc-400">Bloco do E-mail</label>
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={globalStyles.containerColor}
                    onChange={(e) => onUpdateGlobalStyles({ ...globalStyles, containerColor: e.target.value })}
                    className="w-6.5 h-6.5 rounded border border-zinc-800 bg-transparent cursor-pointer"
                  />
                  <span className="text-xs font-mono text-zinc-400 uppercase">{globalStyles.containerColor}</span>
                </div>
              </div>
